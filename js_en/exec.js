...
d3.select('#switchTables').on('click',d=>{
    if(!table_mode){
        ...
        d3.select('#switchTables').html('&harrw;&nbsp;back to detailed table')
        table_mode = true
    }else{
        d3.select('#preview').style('display','block')
        d3.select('#preview_group').style('display','none')
        d3.select('#switchTables').html('&harrw;&nbsp;table with grouped recipients')
        table_mode = false
    }
})
...
var state = {}, initialised = false, searchterm_name = '', empfaenger, organisations_a, organisations, organisations_keys, searchterm_zweck = '', overall_count, overall_sum, amount, bin_borders, bin_count_borders, bin_sum_borders, dates_count, fund, all, all_groups, date, dates, names, zwecke, empfaenger_root, names_sum, names_count, postcode, postcodes_count, postcodes, politikbereich, politikbereichs, geber, gebers, geber_jahre, gebers_jahre, politikbereich_jahre, politikbereichs_jahre, postcodeYears, postcodeYears_count, filter, sum_chart, art_chart, year_chart, map_chart, map_chart_years, table;
...
function buildVis(){
  ...
  bin_chart = binChart(d3.select('#binChart-1'), calcBins(amount, bin_borders, 'betrag'), 'Grants', filterFunction, 'bin');
  bin_chart.init();

  let organisations_t = buildOrganisation()

  bin_sum_chart = binChart(d3.select('#binChart-2'), calcBinsSpecial(organisations_t, bin_sum_borders, 'sum'), 'Recipients with sum', filterFunction, 'bin_sum');
  bin_sum_chart.init();

  bin_count_chart = binChart(d3.select('#binChart-3'), calcBinsSpecial(organisations_t, bin_count_borders, 'count'), 'Recipients', filterFunction, 'bin_count');
  ...
  d3.select('#showMiniMap').on('click', ()=>{
    if(!minimapstate){
        ...
        d3.select('#showMiniMap').html('&#10224;&nbsp;hide annual maps')
        map_chart_years.forEach(m=>{m.show();})
    }else{
        d3.select('#showMiniMap').html('&#10225;&nbsp;show annual maps')
