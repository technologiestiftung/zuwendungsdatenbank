var binChart = function(_container, _bins, _title, _filterFunction, _filterKey){
  ...
  if(data[0].from == 1){
    chart_labels.append('tspan').text(d=>((d.to > 1)?(' Recipient of '+d.from+((d.to == d.from)?'':(' to '+d.to))+' projects'):' Recipient of one project'))
  }else{
    chart_labels.append('tspan').text(d=>` ${title} ${(d.from != d.to)?('between '+currency(d.from,true)+' and '+currency(d.to,true)):('of '+currency(d.to,true))}`)
  }

  function formatNum (val, ext) {
    if(val >= 1000000000){
      return (val/1000000000).toFixed(2) + ((ext)?' Bil. €':'');
    }else if(val >= 1000000){
      return (val/1000000).toFixed(0) + ((ext)?' Mil. €':'');
    }else if(val >= 1000){
      return (val/1000).toFixed(2) + ((ext)?' K €':'');
    }else{
      return val;
    }
  }
