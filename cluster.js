...
function cleanData(fileName){
	...
	new Promise(function(resolve, reject){
		...
		var cols = ["geber","art","politikbereich"],
			tests = {},
			colCluster = {},
			clusterKeys = {}
		cols.forEach(col => {
		...
		csv.forEach(c => {
			...
			cols.forEach(col => {
				...
				if(v in clusterKeys[col]){
					...
					colCluster[col][clusterKeys[col][v]].years[y-yearRange[0]] = +m
				}else{
					...
					if(r && r[0][0] > 0.9){
						...
						let match1 = data.match(new RegExp(v, 'g')),
							match2 = data.match(new RegExp(r[0][1], 'g'))
						...
						colCluster[col][clusterKeys[col][r[0][1]]].years[y-yearRange[0]] = +m
						...
						for(var year = 0; year <= yearRange[1]-yearRange[0]; year++){
							colCluster[col][clusterKeys[col][v]].years.push(0)
						}
