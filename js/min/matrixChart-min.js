...
var matrixChart = function matrixChart(_container, _labels, _data, _dict, _count, _filterFunction, _filterKey, _tooltip) {
  //ADD SUM
  var module = {},
      dict = _dict,
      dict_keys = {};
  ...
  var init = true,
      ...
      years = [],
      sums = [],
      sumw = d3.scaleLinear().domain([0, Math.pow(d3.max(d3Data, function (d) {
    return d.sum;
  }), root)]),
      sumcountw = d3.scaleLinear().domain([0, Math.pow(d3.max(d3Data, function (d) {
    return d.count;
  }), root)]),
      g = svg.append('g').attr('transform', 'translate(' + (padding + xOffset) + ',' + (padding + yOffset) + ')'),
      groups = g.selectAll('g').data(d3Data).enter().append('g').attr('class', 'matrixGroup'),
      yLabels = groups.append('text').datum(function (d, i) {
    return labels.all()[i];
  }).attr('text-anchor', 'end'),
      bgs = groups.selectAll('rect.bg').data(function (d) {
    return d.years;
  }).enter().append('rect').classed('bg', true),
      bars = groups.selectAll('rect.bar').data(function (d) {
    return d.years;
  }).enter().append('rect').classed('bar', true),
      bars_count = groups.selectAll('rect.count').data(function (d) {
    return d.years;
  }).enter().append('rect').classed('count', true),
      sumg = groups.selectAll('g.sumgroup').data(function (d) {
    return [d];
  }).enter().append('g').classed('sumgroup', true),
      sumbg = sumg.append('rect').attr('height', blockHeight).classed('sumbg', true),
      sumbars = sumg.selectAll('rect.sumbar').data(function (d) {
    return [d];
  }).enter().append('rect').attr('height', blockHeight / 2).classed('sumbar', true),
      sumbars_count = sumg.selectAll('rect.count').data(function (d) {
    return [d];
  }).enter().append('rect').attr('x', 0).attr('height', blockHeight / 2).attr('y', blockHeight / 2).classed('count', true),
  ...
  defs.selectAll('pattern').data(['texture', 'texture_blue', 'texture_grey']).enter().append('pattern').attr('id', function (d) {
    return d;
  }).attr('width', 5).attr('height', 5).attr('patternUnits', 'userSpaceOnUse').append('image').attr('xlink:href', function (d) {
    return './images/' + d + '.png';
  }).attr('width', 5).attr('height', 5);

  module.updateToolTip = function (x, y, d) {
    ...
    var tableStr = '<tr><th>Jahr</th><th>Summe</th><th>Anzahl</th></tr>';
    d.years.forEach(function (y) {
      tableStr += "<tr><td>".concat(y.key, "</td><td>").concat(currency(y.value), "</td><td>").concat(y.count, "</td></tr>");
    });
    tooltip.direction('vertical');
    tooltip.show({
      title: d.label,
      body: "Summe in (\u20AC):<br /><i>".concat(currency(d.sum)) + (showPercentage ? "&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;".concat((d.sum / all.value() * 100).toFixed(2), "%") : '') + "</i><br><br />Anzahl F\xF6rderprojekte:<br /><i>".concat(d.count) + (showPercentage ? "&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;".concat((d.count / all_groups.value() * 100).toFixed(2), "%") : '') + "</i><br /><br /><table>".concat(tableStr, "</table>"),
      x: x,
      y: y
    });
  };
  ...
  function splitData(_data, _count, _labels) {
    ...
    _data.all().forEach(function (d, di) {
      ...
      ndata[nkeys[id]].years.push({
        key: year,
        value: d.value,
        count: _count.all()[di].value
      });
    });
    ...
    ndata.forEach(function (d, di) {
      ...
      ndata[di]['sum'] = ndata[di].years.reduce(function (sum, v) {
        return v.value + sum;
      }, 0);
      ndata[di]['count'] = ndata[di].years.reduce(function (sum, v) {
        return v.count + sum;
      }, 0);
      sorter.push({
        id: di,
        label: ndata[di]['label'].trim(),
        sum: ndata[di]['sum'],
        count: ndata[di]['count']
      });
    });
  ...
  module.init = function () {
    ...
    xLabels = xLabels.selectAll('text').data(years.concat('Summe')).enter().append('text').attr('text-anchor', 'middle').text(function (d) {
      return d;
    ...
    sortButton.append('tspan').text('Name').classed('sort-label', true).on('click', function () {
      module.toggleSort('label');
    });
    ...
    sortButton.append('tspan').text('Summe').classed('sort-sum', true).on('click', function () {
      module.toggleSort('sum');
    });
    sortButton.append('tspan').text(' / ');
    sortButton.append('tspan').text('↑').classed('sort-d-count', true);
    sortButton.append('tspan').text('↓').classed('sort-d-icount', true);
    sortButton.append('tspan').text('Anzahl').classed('sort-count', true).on('click', function () {
      module.toggleSort('count');
    });
    module.resize();
    module.sortItems();
  };
  ...
  module.resize = function () {
    ...
    sumbg.transition().duration(init ? 0 : 500).attr('width', blockWidth);
    xLabels.transition().duration(init ? 0 : 500).attr('x', function (d, i) {
      return (blockWidth + (padding - (i == years.length ? 0 : 1))) * i + blockWidth / 2;
    });
    bgs.data(function (d) {
      return d.years;
    }).transition().duration(init ? 0 : 500).attr('width', blockWidth).attr('height', blockHeight).style('fill', function (d) {
      return filterYears.indexOf(d.key) > -1 ? 'rgba(45,145,210,0.3)' : '';
    }).attr('x', function (d) {
      return (blockWidth + (padding - 1)) * years.indexOf(d.key);
    });
    buttons.attr('height', blockHeight).attr('width', (blockWidth + padding) * (years.length + 1) + xOffset);
    sumg.transition().duration(init ? 0 : 500).attr('transform', 'translate(' + years.length * (padding + blockWidth) + ',0)');
    module.update();
  };
  ...
  module.sortItems = function () {
    groups.data(d3Data).classed('selected', function (d) {
      ...
      return false;
    }).transition().duration(init ? 0 : 500).attr('transform', function (d) {
  ...
  module.update = function () {
    ...
    groups.data(d3Data);
    buttons.data(d3Data);
    sumg.data(function (d) {
      return [d];
    });
    sumbars.data(function (d) {
      return [d];
    }).transition().duration(init ? 0 : 500).attr('width', function (d, i) {
      return sumw(Math.pow(d.sum, root));
    });
    sumbars_count.data(function (d) {
      return [d];
    }).transition().duration(init ? 0 : 500).attr('width', function (d, i) {
    ...
    bars.data(function (d) {
      return d.years;
    }).transition().duration(init ? 0 : 500).attr('width', blockWidth / 2).attr('height', function (d) {
  ...
  module.data = function (_data, _labels, _count, _filters, _filterYears) {
    ...
    sumw.domain([0, Math.pow(d3.max(d3Data, function (d) {
      return d.sum;
    }), root)]);
    sumcountw.domain([0, Math.pow(d3.max(d3Data, function (d) {
      return d.count;
    }), root)]);
    module.update();
  };
...
//# sourceMappingURL=matrixChart-min.js.map
