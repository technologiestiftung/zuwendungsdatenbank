...
var yearChart = function yearChart(_container, _dates, _counts, _filterFunction, _filterKey, _tooltip) {
  var module = {},
      tooltip = _tooltip,
  ...
  var g = svg.append('g').attr('transform', 'translate(' + (padding + xOffset) + ',' + 2 * padding + ')'),
  ...
  }).classed('dategroup', true).on('mousemove', function (d) {
    tooltip.move({
      x: d3.event.pageX,
      y: d3.event.pageY
    });
  ...
  function updateToolTip(x, y, d) {
    ...
    tooltip.show({
      title: d.count.key,
      body: "Summe in \u20AC:<br /><i>".concat(currency(d.date.value)) + (showPercentage ? "&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;".concat((d.date.value / all.value() * 100).toFixed(2), "%") : '') + "</i><br><br />Anzahl F\xF6rderprojekte:<br /><i>".concat(d.count.value) + (showPercentage ? "&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;".concat((d.count.value / all_groups.value() * 100).toFixed(2), "%") : '') + "</i>",
      x: x,
      y: y
    });
  }
  ...
  module.update = function () {
    y.domain([0, d3.max(dates.all(), function (d) {
      return d.value;
    })]);
    y_count.domain([0, d3.max(counts.all(), function (d) {
      return d.value;
    })]);
    chart.data(data).classed('selected', function (d) {
      if (filters.indexOf(d.date.key) > -1) {
        return true;
      }

      return false;
    }).transition().duration(init ? 0 : 500).attr('transform', function (d, i) {
      return 'translate(' + x(d.date.key) + ',0)';
    });
    chart_dates.transition().duration(init ? 0 : 500).attr('y', function (d, i) {
      return y(d.date.value);
    }).attr('width', function (d, i) {
      return barWidth / 2;
    }).attr('height', function (d, i) {
      return height - 3 * padding - yOffset - y(d.date.value);
    });
    chart_counts.transition().duration(init ? 0 : 500).attr('x', barWidth / 2).attr('y', function (d, i) {
      return y_count(d.count.value);
    }).attr('width', function (d, i) {
      return barWidth / 2;
    }).attr('height', function (d, i) {
      return height - 3 * padding - yOffset - y_count(d.count.value);
    });
    y_axis.transition().duration(init ? 0 : 500).call(yAxis);
    y_count_axis.transition().duration(init ? 0 : 500).call(yCountAxis);
    x_axis.transition().duration(init ? 0 : 500).call(xAxis);
    init = false;
  };
...
//# sourceMappingURL=yearChart-min.js.map
